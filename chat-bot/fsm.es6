...
export async function step(states, last, input) {
  ...
  // check the proxy
  if (state) {
    if (debug) console.tag('fsm').log('Attempt PROXY.transitions');
    ...
    if (!output && next && state) {
      if (debug) console.tag('fsm').log('Attempt transitionTo', next);
  ...
  if (!next && !output && state) {
    if (debug) console.tag('fsm').log('Attempt LAST_STATE.transitions');
  ...
  if (!next && !output && state) {
    if (debug) console.tag('fsm').log('Attempt FALLBACK.transitions');
  ...
  if (!output && next && state) {
    if (debug) console.tag('fsm').log('Attempt LAST_STATE.process', next);
  ...
  if (debug) console.tag('fsm').log(`Stepped ${last} → ${next}`);
  if (debug) console.tag('fsm').log('Output', output);
